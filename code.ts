...
function listAllLayerNames(node: SceneNode, depth: number = 0): void {
...
function extractComponentsFromNode(node: SceneNode): SceneNode[] {
...
function formatPriceValue(value: any, propertyPath: string): string {
...
// posted message.
figma.ui.onmessage = async (msg: { type: string; mappings?: Record<string, string>; selectedData?: any[]; imageProperties?: string[] }) => {
  if (msg.type === 'apply-to-figma') {
    ...
    const componentUpdatePromises = allComponents.map(async (component, index) => {
      ...
      const mappingPromises = Object.entries(msg.mappings || {}).map(async ([propertyPath, layerName]: [string, string]) => {
        try {
